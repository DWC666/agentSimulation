...
public class SimulationMain {
    public static final double DOUBLE_MAX = 99999999.0;
    private static final int nodeNum = 92; //节点数量
    private static final int linkNum = 362; //路段数量
    private static int carLinkNum = 102; //汽车路段数量
    public static final int parkingLotNum = 7; //停车场数量

    private static final int originNum = 3; //起点数量
    ...
    public static  double metroTicketPrice  = 4 * 0.1; //地铁票价（元）
    public static  double busTicketPrice  = 2 * 0.1; //公交票价（元）
    ...
    //停车场泊位总量
    private static final int[] parkingLotCapacity = {1000, 1000, 1000, 500, 500, 500, 500};
    //可预约泊位占总泊位数量的比例
    ...
    private static final int[][] parkingLotIdOfCarAndWalk = {{31, 47}, {28, 34}, {32, 55}, {13, 51},  {20, 67}, {19, 59}, {21, 60}};
    //停车场节点相邻的汽车网络节点
    private static final Integer[][] adjacentCarNodes = {{10, 30}, {2, 29}, {16, 33},  {12, 14}, {12, 14}, {12, 18, 21, 25}, {14, 19, 22, 26}};
    //换乘停车场id
    private static final Set<Integer> pAndRPark = new HashSet<>(Arrays.asList(new Integer[]{0, 1, 2}));

    private static final int[] originalNodes = {35, 46, 56}; //出发节点集合
    private static final int[] destinationNodes = {50, 52}; //目标节点集合
...
//                    logger.info(    "已使用的普通泊位数量：" + parkingLot.getUsedGeneralBerth());
        ...
        for(int key : finishedTravellerMap.keySet()){
            ...
            if(traveller.getStartParkTime() - traveller.getFirstArriveParkingLotTime() > 0){
                parkSearchTime += (traveller.getStartParkTime() - traveller.getFirstArriveParkingLotTime());
                parkSearchCount++;
            }
            if(traveller.isCommuter()){
                commuterCount++;
        ...
        data[1][count] = parkSearchTime / parkSearchCount; //平均巡游时间
        data[2][count] = parkSearchCount * 1.0 / drivingCount; //停车巡游比例
        data[3][count] = (finishedTravellerMap.size() - drivingCount) * 1.0 / finishedTravellerMap.size(); //停车换乘出行比例
        data[4][count] = totalMetroAndBusDistance / (totalMetroAndBusDistance + totalCarDistance); //公共交通分担率
        ...
        System.out.println("预约停车人数：" + bookParkUser + " 占比 " + (bookParkUser*1.0 / idGenerator.get()));
        System.out.println("正在排队停车人数：" + queuingNum);
        System.out.println("巡游比例：" + (parkSearchCount * 1.0 / drivingCount));
        System.out.println("平均巡游时间：" + (parkSearchTime / parkSearchCount));
        System.out.println("平均车公里：" + avgCarDistance);
...
//                    throw new RuntimeException("出行者" + traveller.getId() + "的路径过长，存在异常：" + traveller);
        ...
        avgCarDistance /= finishedTravellerMap.size();
//        double result = totalMetroAndBusDistance / (totalCarDistance + totalMetroAndBusDistance);
        ...
        return totalMetroAndBusDistance / (totalCarDistance + totalMetroAndBusDistance);
    }

    public static double totalCarDistance = 0; //车公里数
    public static double totalMetroAndBusDistance = 0; //公共交通公里数
    public static double[][] data = new double[5][12];
